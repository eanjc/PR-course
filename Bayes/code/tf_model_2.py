...
w1=tf.Variable(tf.random_normal([28*28,2048],stddev=1,seed=1))
b1=tf.Variable(tf.random_normal([2048],stddev=1,seed=1))

t1=tf.nn.relu(tf.matmul(x,w1)+b1)

w2=tf.Variable(tf.random_normal([2048,10],stddev=1,seed=1))
b2=tf.Variable(tf.random_normal([10],stddev=1,seed=1))

t2=tf.nn.relu(tf.matmul(t1,w2)+b2)


bs=tf.Variable(tf.random_normal([10],stddev=1,seed=1))
yy=tf.nn.softmax(t2+bs)
